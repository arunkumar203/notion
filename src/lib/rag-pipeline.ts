...
export class RAGPipeline {
    ...
    private cachedGenModel: string | null = null;
    ...
    // Get the configured AI model from admin settings
    private async getGenModel(): Promise<string> {
        if (this.cachedGenModel) return this.cachedGenModel;

        try {
            const snapshot = await this.rtdb.ref('adminSettings/aiFlashModel').once('value');
            const model = snapshot.val();
            if (model && typeof model === 'string') {
                this.cachedGenModel = model;
                return model;
            }
        } catch (error) {
            console.warn('Could not fetch AI model from settings, using default:', error);
        }
...
ANSWER (using ONLY the context above):`;
            ...
            // Generate response
            const genModelName = await this.getGenModel();
            const model = this.genaiClient.getGenerativeModel({ model: genModelName });
