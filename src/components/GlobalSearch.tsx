...
import { FiSearch, FiX, FiBook, FiFolder, FiFileText, FiLayers, FiBriefcase } from 'react-icons/fi';
...
export default function GlobalSearch({ onNavigate }: GlobalSearchProps) {
    ...
    const performSearch = useCallback(async (searchQuery: string) => {
        ...
        try {
            ...
            // Add workspaces
            searchData.workspaces.forEach(workspace => {
                searchResults.push({
                    id: workspace.id,
                    name: workspace.name,
                    type: 'workspace'
            ...
            searchData.notebooks.forEach(notebook => {
                searchResults.push({
                    ...
                    workspaceName: notebook.workspaceName
                });
            });
            ...
            // Sort results by type priority: workspaces, notebooks, sections, topics, pages
            const typeOrder = { workspace: 0, notebook: 1, section: 2, topic: 3, page: 4 };
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            case 'workspace': return FiBriefcase;
    ...
    // Get breadcrumb path for result
    const getBreadcrumb = (result: SearchResult): string => {
        switch (result.type) {
            case 'workspace':
                return '';
            case 'notebook':
                return result.workspaceName ? `in ${result.workspaceName}` : '';
            case 'section':
                return result.workspaceName && result.notebookName
                    ? `in ${result.workspaceName} → ${result.notebookName}`
                    : result.notebookName ? `in ${result.notebookName}` : '';
            case 'topic':
                return result.workspaceName && result.notebookName && result.sectionName
                    ? `in ${result.workspaceName} → ${result.notebookName} → ${result.sectionName}`
                    : result.notebookName && result.sectionName
                        ? `in ${result.notebookName} → ${result.sectionName}` : '';
            case 'page':
                return result.workspaceName && result.notebookName && result.sectionName && result.topicName
                    ? `in ${result.workspaceName} → ${result.notebookName} → ${result.sectionName} → ${result.topicName}`
                    : result.notebookName && result.sectionName && result.topicName
                        ? `in ${result.notebookName} → ${result.sectionName} → ${result.topicName}` : '';
            default:
                return '';
        }
    };
        ...
        <div ref={searchRef} className="relative flex-1 max-w-md mx-4">
            <div className="relative">
                ...
                <input
                    ...
                    placeholder="Search workspaces, notebooks, sections, topics, pages..."
