...
import { appwriteClient, appwriteStorage, AppwriteID } from "@/lib/appwrite";
...
export default function Uploader({ onDone }: { onDone?: () => void }) {
  ...
  const uploadOne = async (it: UploadItem) => {
    ...
    if (!bucketId) {
      setItems((prev) =>
        prev.map((p) =>
          p.id === it.id
            ? { ...p, status: "error", error: "Bucket not configured" }
            : p
        )
      );
      return;
  ...
  startFakeProgress(it.id, it.file.size);
    ...
    try {
      ...
      const { jwt, error } = await res.json();
      if (!res.ok || !jwt) throw new Error(error || "Failed to fetch upload token");
      appwriteClient.setJWT(jwt);

      await appwriteStorage.createFile(bucketId, AppwriteID.unique(), it.file);
