...
export async function ensureSampleWorkspace(uid: string) {
  ...
  // 1) Notebook
  const nbRef = push(ref(rtdb, `notebooks`));
  ...
  await set(ref(rtdb, `users/${uid}/notebooks/${notebookId}`), {
    name: 'Sample Notebook',
    createdAt: now,
    updatedAt: now,
  });
  ...
  // 2) Section
  const sectionRef = push(ref(rtdb, `notebooks/${notebookId}/sections`));
  const sectionId = sectionRef.key as string;
  await set(ref(rtdb, `notebooks/${notebookId}/sections/${sectionId}`), {
    owner: uid,
    name: 'Sample Section',
    createdAt: now,
    updatedAt: now,
  ...
  // 3) Topic
  const topicRef = push(ref(rtdb, `notebooks/${notebookId}/sections/${sectionId}/topics`));
  const topicId = topicRef.key as string;
  await set(ref(rtdb, `notebooks/${notebookId}/sections/${sectionId}/topics/${topicId}`), {
    owner: uid,
    name: 'Sample Topic',
    createdAt: now,
    updatedAt: now,
  ...
  const createPage = async (name: string, parentPageId?: string | null, order = 0, html?: string) => {
    const pageRef = push(ref(rtdb, `notebooks/${notebookId}/sections/${sectionId}/topics/${topicId}/pages`));
    const pageId = pageRef.key as string;
    await set(ref(rtdb, `notebooks/${notebookId}/sections/${sectionId}/topics/${topicId}/pages/${pageId}`), {
      owner: uid,
      name,
      createdAt: now,
      updatedAt: now,
  ...
  const sampleYouTubeId = 'dQw4w9WgXcQ';
  const sampleHtml = `
  ...
  await setDoc(rootDoc, {
    content: `
      ...
      <h3>App features</h3>
      <ul>
        <li>Notebook hierarchy:notebooks>sections>topics>pages>child pages</li>
        <li>secret notes</li>
