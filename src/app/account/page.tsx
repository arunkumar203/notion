...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    if (lastSentRef.current && lastSentRef.current.apiKey === pending.apiKey && lastSentRef.current.model === pending.model && lastSentRef.current.speed === pending.speed) {
      return;
    }
    // Only save when apiKey is non-empty to avoid server 400 during typing/clears
    if (!pending.apiKey.trim()) {
      setAiSaving(false);
      return;
    }
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 401) {
            ...
            console.warn('Failed to save AI settings', j?.error || res.statusText);
          }
        } else {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (!deletePassword) return notify('Please enter your password to confirm account deletion');
    ...
    try {
      ...
      await signOut();
      notify('Your account has been deleted successfully');
      router.push('/');
    } catch (err: any) {
