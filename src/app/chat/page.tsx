...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    const shouldIncludeThinking = selectedModel === 'pro' || thinkingEnabled;
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          model: selectedModel === 'pro' ? proModelName : flashModelName,
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <select
              value={selectedModel}
              onChange={(e) =>
                setSelectedModel(e.target.value as 'flash' | 'pro')
              }
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                role="switch"
                aria-checked={selectedModel === 'pro' ? true : thinkingEnabled}
                onClick={() => {
                  if (selectedModel === 'pro') return; // Pro model always includes thoughts
                  setThinkingEnabled(!thinkingEnabled);
                }}
                disabled={isGenerating || selectedModel === 'pro'}
                className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-200 ${selectedModel === 'pro' || thinkingEnabled ? 'bg-indigo-600' : 'bg-gray-300'
                  } ${selectedModel === 'pro' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                  }`}
                title={
                  selectedModel === 'pro'
              ...
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition duration-200 ${selectedModel === 'pro' || thinkingEnabled ? 'translate-x-4' : 'translate-x-1'
                    }`}
                />
              </button>
            </div>
