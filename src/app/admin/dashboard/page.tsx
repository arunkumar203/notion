...
import { FiUsers, FiFolder, FiBook, FiLayers, FiTag, FiFileText, FiSettings, FiBarChart } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        // Fetch workspaces and count notebooks, sections, topics, pages
        const workspacesRef = ref(rtdb, 'workspaces');
        const workspacesSnapshot = await get(workspacesRef);
        const workspacesData = workspacesSnapshot.exists() ? workspacesSnapshot.val() : {};

        const totalWorkspaces = Object.keys(workspacesData).length;
        ...
        if (workspacesData && typeof workspacesData === 'object') {
          Object.values(workspacesData).forEach((workspace: any) => {
            if (workspace && typeof workspace === 'object' && workspace.notebooks) {
              Object.values(workspace.notebooks).forEach((notebook: any) => {
