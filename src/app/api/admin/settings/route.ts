  ...
  try {
    ...
    try {
      ...
      const settings = {
        ...
        aiFlashModel: rawSettings?.aiFlashModel ?? 'gemini-2.5-flash',
        aiProModel: rawSettings?.aiProModel ?? 'gemini-2.5-pro',
      };
...
export async function POST(request: Request) {
  try {
    ...
    const { emailSendingEnabled, showCreatorAttribution, homePageMessage, onboardingMandatory, allowNewUserSignup, maintenanceMode, maintenanceMessage, aiFlashModel, aiProModel } = body || {};

    if (typeof emailSendingEnabled !== 'boolean' && typeof showCreatorAttribution !== 'boolean' && typeof homePageMessage !== 'string' && typeof onboardingMandatory !== 'boolean' && typeof allowNewUserSignup !== 'boolean' && typeof maintenanceMode !== 'boolean' && typeof maintenanceMessage !== 'string' && typeof aiFlashModel !== 'string' && typeof aiProModel !== 'string') {
    ...
    try {
      ...
      if (typeof aiFlashModel === 'string' && aiFlashModel.trim()) {
        updates.aiFlashModel = aiFlashModel.trim();
      }

      if (typeof aiProModel === 'string' && aiProModel.trim()) {
        updates.aiProModel = aiProModel.trim();
      }
      ...
      if (typeof aiFlashModel === 'string') {
        responseSettings.aiFlashModel = aiFlashModel;
      }
      if (typeof aiProModel === 'string') {
        responseSettings.aiProModel = aiProModel;
      }
