...
import { FiAlertCircle, FiPlus, FiLogOut, FiTrash2, FiMenu, FiCopy, FiCheck, FiX, FiInfo, FiShare2, FiLink, FiLock, FiChevronRight } from 'react-icons/fi';
...
export default function NotebooksPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(r, (snap) => {
      ...
      setOnboardingInProgress(Boolean(v && v.inProgress));
  ...
  }, [selectedPage, getPageContent, onboardingInProgress, pages]);

  // Keep editable title in sync with selected page and subscribe to share status live
  useEffect(() => {
    const id = getPageId(selectedPage);
  const current = id ? pages.find((p: Page) => p.id === id) : null;
  ...
  const uiPages = useMemo(() => {
    ...
    if (!isTitleEditing) return pages;
    const newName = pageTitle;
  return pages.map((p: Page) => (p.id === id ? { ...p, name: newName } : p));
  ...
  // NEW: Memoize the final, deduplicated, and sorted pages list for the Panel
  const memoizedPanelPages = useMemo(() => {
    const currentPages = uiPages.slice(); // Create a shallow copy to prevent mutation issues during sort/filter
  ...
  const deduplicatedPages = currentPages.filter((p: Page) => {
    ...
    // Sort
  return deduplicatedPages.sort((a: Page, b: Page) => {
    // Stable sort: custom order, then createdAt, then id
  ...
  }, [uiPages]); // Depend only on uiPages, which itself is memoized.
...
function UserMenu({ email, onLogout }: { email: string; onLogout: () => Promise<void> | void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-md shadow-lg z-60">
          <Link
            href="/account"
