...
import { FiSettings, FiArrowLeft, FiMail, FiCheck, FiX, FiUser, FiDatabase, FiCpu } from 'react-icons/fi';
...
export default function AdminSettingsPage() {
        ...
        const fetchSettings = async () => {
            ...
            try {
                ...
                setSettings({
                    ...
                    aiFlashModel: data.settings?.aiFlashModel ?? 'gemini-2.5-flash',
                    aiProModel: data.settings?.aiProModel ?? 'gemini-2.5-pro',
                });
            } catch (error: any) {
        ...
        const fetchStorageLimit = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    setStorageLimitMB(data.limitMB || 50);
                }
            } catch (e) {
                console.error('Error fetching storage limit:', e);
            }
        };
        fetchStorageLimit();
    }, [user, canAccessAdmin]);
    ...
    const handleSaveStorageLimit = async () => {
        if (!isRootAdmin || savingStorage) return;
        ...
        try {
            const response = await fetch('/api/admin/storage-limit', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ limitMB: storageLimitMB }),
                credentials: 'include'
            });
            ...
            if (response.ok) {
                setSuccess(`Storage limit updated to ${storageLimitMB} MB`);
                setTimeout(() => setSuccess(null), 3000);
            } else {
                setError(result.error || 'Failed to update storage limit');
            }
        } catch (error: any) {
            setError(`Error: ${error.message}`);
        } finally {
    ...
    const handleSaveAIModels = async () => {
        if (!isRootAdmin || savingAIModels) return;

        setSavingAIModels(true);
        setError(null);
        setSuccess(null);
        ...
        try {
            const response = await fetch('/api/admin/settings', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    aiFlashModel: settings.aiFlashModel,
                    aiProModel: settings.aiProModel,
                }),
                credentials: 'include'
            });
            ...
            if (response.ok) {
                ...
                setError(result.error || 'Failed to save AI model configuration');
            }
        } catch (error: any) {
            setError(`Error: ${error.message}`);
        } finally {
            setSavingAIModels(false);
        }
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-gradient-to-br from-cyan-50 to-teal-50 shadow-lg rounded-2xl border border-cyan-100 p-6 hover:shadow-xl transition-shadow duration-300">
                            <div className="flex items-center mb-6">
                                ...
                                <div className="ml-4">
                                    <h3 className="text-xl font-bold text-gray-900">Storage Limits</h3>
                                    <p className="text-sm text-cyan-600 font-medium">Per-User File Storage</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="bg-white/70 backdrop-blur-sm rounded-xl p-4 border border-white/50">
                                    <div className="mb-3">
                                        <h4 className="text-sm font-semibold text-gray-900">Default Storage Limit</h4>
                                        <p className="text-xs text-gray-600 mt-1">Maximum file storage space per user</p>
                                    </div>
                                    <div className="flex items-center space-x-3">
                                        <input
                                            type="number"
                                            ...
                                            onChange={(e) => setStorageLimitMB(Math.max(1, Math.min(10240, parseInt(e.target.value) || 50)))}
                                            disabled={!isRootAdmin}
                                            className="w-24 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:bg-gray-100 disabled:text-gray-500"
                                        />
                                        <span className="text-sm text-gray-600">MB</span>
                                        <span className="text-xs text-gray-500">({storageLimitMB >= 1024 ? `${(storageLimitMB / 1024).toFixed(1)} GB` : `${storageLimitMB} MB`})</span>
                                        <button
                                            onClick={handleSaveStorageLimit}
                                            disabled={savingStorage || !isRootAdmin}
                                            className={`px-4 py-2 text-xs font-medium rounded-lg transition-colors ${savingStorage || !isRootAdmin ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-cyan-600 text-white hover:bg-cyan-700'}`}
                                        >
                                            {savingStorage ? 'Saving...' : 'Save'}
                                        </button>
                                    </div>
                                    <div className="text-xs text-gray-500 bg-gray-50 rounded-lg p-3 mt-3">
                                        <strong>How it works:</strong> Each user can upload files up to this limit. When a user exceeds their quota, they'll see a clear error message.<br />
                                        <strong>Note:</strong> Changing this limit affects all users. Existing files are not deleted if limit is reduced.
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gradient-to-br from-violet-50 to-purple-50 shadow-lg rounded-2xl border border-violet-100 p-6 hover:shadow-xl transition-shadow duration-300">
                            <div className="flex items-center mb-6">
                                ...
                                <div className="ml-4">
                                    <h3 className="text-xl font-bold text-gray-900">AI Models</h3>
                                    <p className="text-sm text-violet-600 font-medium">Gemini Model Configuration</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="bg-white/70 backdrop-blur-sm rounded-xl p-4 border border-white/50">
                                    <div className="mb-4">
                                        <h4 className="text-sm font-semibold text-gray-900 mb-1">System AI Models</h4>
                                        <p className="text-xs text-gray-600">Configure which Gemini models are used for RAG Chat and system AI features.</p>
                                    </div>

                                    <div className="space-y-4">
                                        <div>
                                            <label htmlFor="aiFlashModel" className="block text-xs font-medium text-gray-700 mb-1">
                                                Flash Model (Fast)
                                            </label>
                                            <input
                                                id="aiFlashModel"
                                                type="text"
                                                value={settings.aiFlashModel}
                                                onChange={(e) => setSettings(prev => ({ ...prev, aiFlashModel: e.target.value }))}
                                                disabled={!isRootAdmin}
                                                placeholder="gemini-2.5-flash"
                                                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 disabled:bg-gray-100 disabled:text-gray-500"
                                            />
                                        </div>

                                        <div>
                                            <label htmlFor="aiProModel" className="block text-xs font-medium text-gray-700 mb-1">
                                                Pro Model (Advanced)
                                            </label>
                                            <input
                                                id="aiProModel"
                                                ...
                                                onChange={(e) => setSettings(prev => ({ ...prev, aiProModel: e.target.value }))}
                                                disabled={!isRootAdmin}
                                        ...
                                        <div className="flex justify-end">
                                            <button
                                                ...
                                                disabled={savingAIModels || !isRootAdmin}
                                                className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${savingAIModels || !isRootAdmin ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-violet-600 text-white hover:bg-violet-700'}`}
                                            >
                                                {savingAIModels ? 'Saving...' : 'Save Models'}
                                            </button>
                                        </div>
                                    </div>

                                    <div className="text-xs text-gray-500 bg-gray-50 rounded-lg p-3 mt-4">
                                        <strong>Flash Model:</strong> Used for quick responses in RAG chat.<br />
                                        <strong>Pro Model:</strong> Used for advanced reasoning with thinking mode.<br />
                                        <strong>Note:</strong> Model names must match valid Gemini API model identifiers.
                                    </div>
                                </div>
                            </div>
                        </div>
