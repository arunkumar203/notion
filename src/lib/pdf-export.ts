 ...
 * 2. Transforms MindMap ReactFlow DOM to static tree HTML
 * 3. Overlays drawings if present
 */
function processContentForExport(htmlContent: string, drawings?: string, width: number = 800, height: number = 1000): string {
    ...
    // ========== MINDMAP TRANSFORMATION ==========
    // ReactFlow renders mindmaps with positioned nodes via CSS transforms.
    // The wrapper has data-mindmap attribute with the tree JSON.
    // We convert this to a static tree structure for PDF.
    const mindmapWrappers = tempDiv.querySelectorAll('.mindmap-wrapper[data-mindmap]');
    mindmapWrappers.forEach(wrapper => {
        try {
            const rawData = wrapper.getAttribute('data-mindmap');
            if (!rawData) return;

            const data = JSON.parse(rawData);

            // Recursive function to build static HTML tree with depth
            const buildTreeHtml = (node: any, depth: number = 0): string => {
            ...
            wrapper.outerHTML = treeHtml;
            // console.log('[PDF Export] Transformed mindmap to static tree');
        } catch (e) {
            console.error('[PDF Export] Error transforming mindmap:', e);
        }
    });
