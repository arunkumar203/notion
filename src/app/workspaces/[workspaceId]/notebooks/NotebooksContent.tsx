...
import { FiAlertCircle, FiPlus, FiLogOut, FiTrash2, FiMenu, FiCopy, FiCheck, FiX, FiInfo, FiShare2, FiLink, FiLock, FiChevronRight, FiChevronLeft, FiShield, FiStar, FiDownload } from 'react-icons/fi';
...
import DrawingRibbon, { type ViewPattern } from '@/components/Editor/DrawingRibbon';
...
interface SearchResult {
  id: string;
  name: string;
  type: 'workspace' | 'notebook' | 'section' | 'topic' | 'page';
  workspaceId?: string;
...
const Panel = ({
  ...
  const handleCreate = async () => {
    if (!onCreate) return; // Guard for read-only mode
...
export default function NotebooksContent({ workspaceId }: NotebooksContentProps) {
  ...
  const { role, isOwner, isSharedWorkspace } = useWorkspacePermissions({
    workspaceId,
    userId: user?.uid
  });
  ...
  const canReorder = canEdit; // Only owner/admin/editor can reorder
  const canComment = isOwner || role === 'admin' || role === 'editor' || role === 'commenter';
  const isViewerOrCommenter = role === 'viewer' || role === 'commenter';
  const isViewer = role === 'viewer';
  // Show a read-only editor for viewers and commenters (but allow comments for commenter)
  ...
  const handleExportTopic = async (topicId: string) => {
    ...
    try {
      ...
      const pagesRef = dbRef(rtdb, `workspaces/${workspaceId}/notebooks/${selectedNotebook}/sections/${selectedSection}/topics/${topicId}/pages`);
  ...
  const handleExportSection = async (sectionId: string) => {
    ...
    try {
      ...
      const topicsRef = dbRef(rtdb, `workspaces/${workspaceId}/notebooks/${selectedNotebook}/sections/${sectionId}/topics`);
      ...
      const topicsWithContent = await Promise.all(
        sectionTopics.map(async (topic) => {
          // Fetch all pages for this topic directly from database
          const pagesRef = dbRef(rtdb, `workspaces/${workspaceId}/notebooks/${selectedNotebook}/sections/${sectionId}/topics/${topic.id}/pages`);
          const pagesSnapshot = await get(pagesRef);
          const pagesData = pagesSnapshot.val() || {};
  ...
  const handleExportNotebook = async (notebookId: string) => {
    ...
    try {
      ...
      const sectionsRef = dbRef(rtdb, `workspaces/${workspaceId}/notebooks/${notebookId}/sections`);
      ...
      const sectionsWithContent = await Promise.all(
        notebookSections.map(async (section) => {
          ...
          const topicsRef = dbRef(rtdb, `workspaces/${workspaceId}/notebooks/${notebookId}/sections/${section.id}/topics`);
          const topicsSnapshot = await get(topicsRef);
          const topicsData = topicsSnapshot.val() || {};
          ...
          const topicsWithContent = await Promise.all(
            sectionTopics.map(async (topic) => {
              // Fetch all pages for this topic directly from database
              const pagesRef = dbRef(rtdb, `workspaces/${workspaceId}/notebooks/${notebookId}/sections/${section.id}/topics/${topic.id}/pages`);
              const pagesSnapshot = await get(pagesRef);
              const pagesData = pagesSnapshot.val() || {};
  ...
  // View pattern state (rule lines / grid)
  const [viewPattern, setViewPattern] = useState<ViewPattern>({ type: 'none' });
  ...
  // Generate CSS style for view pattern (rule lines / grid)
  const getViewPatternStyle = (): React.CSSProperties => {
    if (viewPattern.type === 'none') {
      return { backgroundColor: '#fff' };
    }

    if (viewPattern.type === 'rule') {
      ...
      const spacing = spacings[viewPattern.style] || 28;

      // Create SVG pattern for ruled lines
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="100" height="${spacing}">
        <line x1="0" y1="${spacing - 1}" x2="100" y2="${spacing - 1}" stroke="#93c5fd" stroke-width="1"/>
      </svg>`;
    ...
    if (viewPattern.type === 'grid') {
      ...
      const cellSize = sizes[viewPattern.style] || 24;

      // Create SVG pattern for grid
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${cellSize}" height="${cellSize}">
        <line x1="${cellSize}" y1="0" x2="${cellSize}" y2="${cellSize}" stroke="#93c5fd" stroke-width="1"/>
        <line x1="0" y1="${cellSize}" x2="${cellSize}" y2="${cellSize}" stroke="#93c5fd" stroke-width="1"/>
      </svg>`;
  ...
  // Save viewPattern when it changes (page-specific)
  const viewPatternLoadedRef = useRef(false);
  useEffect(() => {
    // Skip saving on initial load (when page changes, viewPattern is reset then loaded)
    if (!viewPatternLoadedRef.current) {
      viewPatternLoadedRef.current = true;
      return;
    }
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const pageRef = doc(db, 'pages', pageId);
        await setDoc(pageRef, {
          viewPattern: JSON.stringify(viewPattern),
          lastUpdated: Date.now()
        }, { merge: true });
      } catch (error) {
        console.error('Error saving viewPattern:', error);
      }
    }, 300);

    return () => clearTimeout(timeoutId);
  }, [viewPattern, selectedPage, user]);

  // Reset viewPatternLoadedRef when page changes
  useEffect(() => {
    viewPatternLoadedRef.current = false;
  }, [selectedPage]);
  ...
  useEffect(() => {
    ...
    setDrawingStrokes([]);
    // Reset viewPattern to none when page changes (will be loaded if saved)
    setViewPattern({ type: 'none' });
    ...
    const loadDrawings = async () => {
      ...
      try {
        ...
        if (pageSnap.exists()) {
          ...
          // Load viewPattern if saved
          if (data.viewPattern) {
            try {
              const pattern = typeof data.viewPattern === 'string'
                ? JSON.parse(data.viewPattern)
                : data.viewPattern;
              setViewPattern(pattern);
            } catch {
              setViewPattern({ type: 'none' });
            }
          }
  ...
  const handleSearchNavigation = useCallback(async (result: SearchResult) => {
    try {
      switch (result.type) {
        case 'workspace':
          // For workspace results, navigate to that workspace's notebooks page
          router.push(`/workspaces/${result.id}/notebooks`);
          break;
  ...
  }, [router, hierarchyOpen, hierarchyVisible, selectNotebook, selectSection, selectTopic, selectPage, gotoPage, openHierarchyOverlay, measureOverlayTop]);
  ...
  const editorConfig = useMemo(() => ({
    ...
    editable: !viewOnlyEnabled && canEdit,
  }), [viewOnlyEnabled, canEdit]);
  ...
  const handleRenamePage = useCallback(async (id: string, name: string) => {
    if (!canEdit) {
      alert('You do not have permission to rename pages.');
  ...
  }, [renamePage, selectedPage, setPageTitle, canEdit]);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 text-gray-900">
      ...
      {/* Role indicator banner for shared workspaces with limited access */}
      {isSharedWorkspace && isViewerOrCommenter && (
        <div className={`px-4 py-2 text-sm flex items-center justify-center gap-2 ${isViewer
          ...
          <span>
            {isViewer
              ? 'You have view-only access to this workspace. You cannot edit any content.'
              : 'You have commenter access. You can view content and add comments, but cannot edit pages.'}
          </span>
        </div>
      )}
      ...
      <div ref={mainRef} className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 bg-white border-r border-gray-200 flex-shrink-0 overflow-hidden text-gray-900">
          <Panel
            ...
            onSelect={selectPage}
            onCreate={canEdit ? handleCreatePage : undefined}
            placeholder={canEdit ? "No pages yet. Create one to get started!" : "No pages available."}
              ...
              <div className="min-h-[500px] flex flex-col max-w-4xl mx-auto w-full">
                ...
                <div className="group mb-1 flex justify-between items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 relative">
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (exportChoice === 'drawings') {
                            ...
                            // If user selected "text" (Text Only), do not include drawings
                            const drawingsArg = exportChoice === 'text' ? undefined : pageDrawings;
                            await exportPageToPDF(pageName, pageContent, drawingsArg, editorWidth, editorHeight);
                          }
                        } catch (error) {
                    ...
                    {/* Share and Move to Secret are only available for workspace owners */}
                    {isOwner && (
                      <div className="flex gap-2 relative">
                        ...
                        {shareOpen && (
                          <div ref={sharePopoverRef} className="absolute right-0 top-full mt-2 w-72 bg-white border border-gray-200 rounded-lg shadow-2xl z-50 p-4" onMouseLeave={() => { /* keep open until click away */ }}>
                            <div className="mb-3 text-sm font-semibold text-gray-900 border-b pb-2">Share page</div>
                            ...
                            {shareId && (
                              <div className="mt-2 flex justify-end">
                                <button
                                  ...
                                  className="w-full inline-flex justify-center items-center gap-2 px-3 py-2 text-xs font-medium rounded-md border border-red-200 text-red-700 bg-red-50 hover:bg-red-100 transition-colors mt-2"
                                >
                ...
                <div className="mb-3 flex justify-end items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      role="switch"
                      aria-checked={viewOnlyEnabled || !canEdit}
                      onClick={() => setViewOnlyEnabled((v) => !v)}
                      disabled={!canEdit}
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-200 ${viewOnlyEnabled || !canEdit ? 'bg-indigo-600' : 'bg-gray-300'} ${!canEdit ? 'opacity-50 cursor-not-allowed' : ''}`}
                      title={!canEdit ? "You don't have permission to edit this page" : "Toggle view only mode"}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition duration-200 ${(viewOnlyEnabled || !canEdit) ? 'translate-x-4' : 'translate-x-1'}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 flex flex-col mt-4 md:mt-6" ref={editorWrapRef}>
                  {/* Drawing Ribbon - Above editor (sticky) - Only shown for users who can edit */}
                  {contentReady && canEdit && (
                    <div className="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm">
                      <DrawingRibbon
                        ...
                        onViewPatternChange={setViewPattern}
                      />
                    </div>
                  )}

                  <div className="flex-1 relative min-h-[calc(100vh-200px)] editor-container" style={{ boxShadow: '0 0 0 1px #e5e7eb', ...getViewPatternStyle() }}>
                    {contentReady ? (
                      <Editor
                        ref={editorRef}
                        content={content}
                        ...
                        config={{
                          ...editorConfig,
                          // Make editor read-only for viewers and commenters
                          editable: canEdit,
                          // Enable commenting for detailed roles
                        ...
                        onRequestAI={canEdit ? () => setAiVisible(true) : undefined}
                      />
                    ) : (
            ...
            <div className="h-[calc(100%-41px)] flex">
              ...
              <div className="w-64 border-r border-gray-200 h-full">
                <Panel
                  ...
                  onCreate={canEdit ? (name) => createNotebook(name) : undefined}
                  placeholder="No notebooks yet"
                  onRename={canEdit ? (id, name) => renameNotebook(id, name) : undefined}
                  onDelete={canEdit ? async (id) => {
              ...
              <div className="w-56 border-r border-gray-200 h-full">
                <Panel
                  ...
                  onCreate={canEdit ? (name) => createSection(name) : undefined}
                  placeholder={selectedNotebook ? 'No sections' : 'Select a notebook'}
                  disabled={!selectedNotebook}
                  onRename={canEdit ? (id, name) => renameSection(id, name) : undefined}
                  onDelete={canEdit ? async (id) => {
                  ...
                  enableDrag={canReorder}
                  onReorder={canReorder ? (ids) => reorderSections(ids) : undefined}
                />
              </div>
              {/* Topics */}
              <div className="w-56 h-full">
                <Panel
                  ...
                  onCreate={canEdit ? async (name: string) => {
                  ...
                  onRename={canEdit ? (id, name) => renameTopic(id, name) : undefined}
                  onDelete={canEdit ? async (id) => {
                  ...
                  enableDrag={canReorder}
                  onReorder={canReorder ? (ids) => reorderTopics(ids) : undefined}
                />
              </div>
            </div>
          </div>
